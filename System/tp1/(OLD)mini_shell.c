...
char *env_variable(char *nom_variable)
{
    ...
    while (*environ)
    {
        ...
        // Comparision
        if (mini_strcmp(nom_s, nom_variable) == 0)
        {
            return s;
        }
        environ++;
    ...
    */
    ...
    if (pid == 0)
    {
        ...
        if (mini_strcmp(args[0], "mini_cd") == 0)
        {
            ...
            if (args[1] == NULL)
            {
                mini_perror("argument error in mini_cd");
            }
            else
            {
                if (chdir(args[1]) == -1)
                {
                    mini_perror("chdir excecution error in mini_cd");
                }
                char cwd[1024];
                if (getcwd(cwd, sizeof(cwd)) == NULL)
                {
                    mini_perror("get current dir error");
                }
        ...
        else if (mini_strcmp(args[0], "mini_env") == 0)
        {
            env_equivalent();
            return 1;
        }
        else if (mini_strcmp(args[0], "mini_getenv") == 0)
        {
            ...
            if (env_variable(args[1]) != NULL)
            {
                mini_printf(env_variable(args[1]));
                mini_printf("\n");
            }
            return 1;
        }
